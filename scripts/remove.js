...
const run = async () => {
    ...
    if (folder) {
        ...
        packageRootInfoEdited.workspaces = packageRootInfoEdited.workspaces.filter(e => e !== `packages/${name}`);
        ...
        tsconfigEdited.include = tsconfigEdited.include.filter(e => e !== `packages/${name}/Tool${toCamelCase(name, true)}.js`);
